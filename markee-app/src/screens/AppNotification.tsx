...
const AppNotification: React.FC = () => {
  ...
  type CreativeDataa = {
    ...
    Sent?: number | undefined | any;
    SessionDuration?: number | undefined | any;
  ...
  const calculateTotalDeliveredForEmailChannel = (creativeData: CreativeCampaign[]): number => {
    return creativeData
      .filter(creativeData => creativeData.Channel === 'App Notification')
  ...
  // const calculateTotalOpenForEmailChannel = (creativeData: CreativeCampaign[]): number => {
  //   return creativeData
  //     .filter(creativeData => creativeData.Channel === 'App Notification')
  //     .flatMap(creativeData => creativeData.Creatives)
  //     .reduce((total, creative) => total + (creative.Open ?? 0), 0);
  // };
  const calculateAverageOpenForEmailChannel = (creativeData: CreativeCampaign[]): number => {
    const filteredCreatives = creativeData
      .filter(data => data.Channel === 'App Notification')
      .flatMap(data => data.Creatives);

    const totalOpen = filteredCreatives.reduce((total, creative) => total + (creative.Open ?? 0), 0);
    const creativeCount = filteredCreatives.length;

    const average = creativeCount > 0 ? totalOpen / creativeCount : 0;
  ...
  const calculateEmailOR = (data: CreativeCampaign[]): number => {
    ...
    data.forEach(campaign => {
      campaign.Creatives.forEach(creative => {
        if (creative.OR !== null && creative.Channel == 'App Notification') {
          emailOR += creative.OR;
          count++
        }
      });
  ...
  const prepareBarData = (campaignIdsToFilter: any[], selectedMetric: any) => {
    const filteredData = creativeData.filter(campaign => campaignIdsToFilter.includes(campaign.CampaignId) && campaign.Channel === "App Notification");
      ...
      campaign.Creatives.forEach(creative => {
        ...
        if (selectedMetric === "ConversionRate") {
          if (!aggregatedData[month].ConversionRate) {
            aggregatedData[month].ConversionRate = 0;
          }
          if (creative.OR !== null) {
            aggregatedData[month].ConversionRate += creative.ConversionRate;
          }
    ...
    const barData: BarData[] = Object.entries(aggregatedData).map(([name, values]) => {
      const avgConversionRate = values.ConversionRate || 0;
      const totalSales = values.TotalSales || 0;

      const result: BarData = { name };

      if (selectedMetric == 'ConversionRate') {
        result.ConversionRate = parseFloat(avgConversionRate.toFixed(1));
      }
  ...
  const prepareTableData = (campaigns: any) => {
    ...
    const summarizedData = campaigns?.map((campaign: any) => {
      const metrics = campaign.creatives.reduce((totals: any, creative: any) => {
        if (creative.channel === "App Notification") {
          totals.Sent += creative.Sent || 0;
          totals.SessionDuration += creative.SessionDuration || 0;
          totals.ConversionRate += creative.ConversionRate || 0;
          totals.TotalSales += creative.TotalSales || 0;
          totals.DeliveryRate += creative.Delivered || 0;
      ...
      return {
        ...
        DeliveryRate: formatNumber(metrics.DeliveryRate),
  ...
  const totalOpen = calculateAverageOpenForEmailChannel(creativeData);
  ...
  const columns = [
    ...
    columnHelper.accessor('DeliveryRate', { header: () => <div className='flex gap-10'><span>Total Deliveries</span></div> }),
    columnHelper.accessor('SessionDuration', { header: () => <div className='flex gap-10'><span>Session Duration</span></div> }),
  ...
  return (
    <Flex gap="4" wrap="wrap">
      <Flex gap="2" align="center" width="100%">
        ...
        <Heading as="h3" className='pagehead'>App Notifications</Heading>
      </Flex>
          ...
          <Tabs.Root defaultValue="performance">
            ...
            <Box pt="3">
                    ...
                    <Flex direction="row" gap="3" align="center" mt='10px' wrap="wrap">
                      ...
                      <Box className='boxy' >
                        <Flex direction="column" gap="2">
                          ...
                          {/* <Box className='numbertext' >
                            <Text size="6" weight="bold" style={{ color: '#FF9800' }}>{totalOpen}</Text>
                          </Box> */}
                          <Box style={{ textAlign: "center", width: "100%" }}>
                            <DonutChart progress={totalOpen} total={100} color={'#6e63d6'} />
                          </Box>
                          <Flex direction="row" gap="3" align="center" >
                            <Box width={'50%'} style={{ textAlign: 'left' }}>
                              <Text>QoQ</Text><br></br>
                              <Text size='2' weight={'bold'}>+8%</Text>
                            </Box>
                            <Box width={'50%'} style={{ textAlign: 'right' }}>
                              <Text>YoY</Text><br></br>
                      ...
                      <Box className='boxy'>
                        <Flex direction="column" gap="2">
                          ...
                          <Flex direction="row" gap="3" align="center" >
                            ...
                            <Box width={'50%'} style={{ textAlign: 'right' }}>
                              <Text>YoY</Text><br></br>
                              <Text size='2' weight={'bold'}>+30%</Text>
                            </Box>
                          </Flex>
                        </Flex>
                      </Box>
                    ...
                    <Card variant="surface" className='card-content' mt='10px' >
                      ...
                      <Flex direction="row" gap={"3"} >
                        ...
                        <Box width='300px' mt='50px'>
                          ...
                          <Box mt='20px'>
                            <MetricFilters type={"App Notification"} campaignData={campaignData} updateData={selectedData} />
                          </Box>
                        </Box>
                        <Box style={{ width: '100%' }}>
                          <BarChartWithLegend
              ...
              <Tabs.Content value="library">
                <View width="100%" UNSAFE_style={{ margin: '10px 0', minHeight:'400px' }}>
                  ...
                  <View UNSAFE_style={{ marginTop: '30PX' }}>
                    ...
                    {filteredData?.filter(campaign =>
                      ...
                      campaign.creatives.some(creative => creative.channel === "App Notification")
                    ).length === 0 && (
                        <Box mt='100px'>
                      ...
                      {/* show total campaigns cound */}
                    <Masonry
                      breakpointCols={breakpointColumnsObj}
                      className="my-masonry-grid"
                      columnClassName="my-masonry-grid_column"
                    >
                      {filteredData?.filter(campaign => campaign.status === "Completed" && campaign.creatives.some(creative => creative.channel === "App Notification")).map((campaign, idx) => {
                        const creativeGraphic = campaign.creatives.find(creative => creative.channel.includes("App Notification"))?.CreativeGraphic;
                        ...
                        <div>
                          {secondaryTrayType === "Performance" && selectedCampaign ? (
                            selectedCampaign.creatives
                              .filter(creative => creative.channel === "App Notification")
                              .map((creative) => {
                      ...
                      <div className="flex jus-center">
                        <div className="flex f-col w-330 shrink-0" style={{ maxWidth: "100%" }}>
                          <div className="flex" style={{ border: "solid 1px black", justifyContent: "flex-start" }}>
                            ...
                            {selectedCampaign && (
                              <img
                                src={selectedCampaign.creatives.find(creative => creative.channel === "App Notification")?.CreativeGraphic}
