...
const Youtube: React.FC = () => {
  ...
  const barConfigs = [
    ...
    { dataKey: 'Likes', fill: '#f8456d' },
    { dataKey: 'Comments', fill: '#7bedff' },
    { dataKey: 'Shares', fill: '#fed473' },
  ...
  type CreativeDataa = {
    ...
    Likes?: number | undefined | any;
    Comments?: number | undefined | any;
    Shares?: number | undefined | any;
  ...
  const calculateTotalReachForEmailChannel = (creativeData: CreativeCampaign[]): number => {
    return creativeData
      .filter(creativeData => creativeData.Channel === 'Youtube')
  ...
  const calculateEmailOR = (data: CreativeCampaign[]): number => {
    ...
    data.forEach(campaign => {
      campaign.Creatives.forEach(creative => {
        if (creative.OR !== null && creative.Channel == 'Youtube') {
          emailOR += creative.OR;
          count++
        }
      });
  ...
  const calculateEmailCTR = (data: CreativeCampaign[]): number => {
    ...
    data.forEach(campaign => {
      campaign.Creatives.forEach(creative => {
        if (creative.CTR !== null && creative.Channel == 'Youtube') {
  ...
  const calculateMarketSpend = (data: CreativeCampaign[]): number => {
    ...
    data.forEach(campaign => {
      campaign.Creatives.forEach(creative => {
        if (creative.MarketSpend !== null && creative.Channel == 'Youtube') {
  ...
  const prepareBarData = (campaignIdsToFilter: any[], selectedMetric: any) => {
    const filteredData = creativeData.filter(campaign => campaignIdsToFilter.includes(campaign.CampaignId) && campaign.Channel === "Youtube");
    ...
    filteredData.forEach(campaign => {
      campaign.Creatives.forEach(creative => {
        ...
        } else if (selectedMetric === "['Likes','Comments','Shares]") {
          if (!aggregatedData[month].Likes) {
            aggregatedData[month].Likes = [];
          }
          if (creative.Like !== null) {
            aggregatedData[month].Likes?.push(creative.Like);


          }
          if (!aggregatedData[month].Comments) {
            aggregatedData[month].Comments = [];
          }
          if (creative.Comment !== null) {
            aggregatedData[month].Comments?.push(creative.Comment);


          }
          if (!aggregatedData[month].Shares) {
            aggregatedData[month].Shares = [];
          }
          if (creative.Shares !== null) {
            aggregatedData[month].Shares?.push(creative.Shares);
    ...
    const barData: BarData[] = Object.entries(aggregatedData).map(([name, values]) => {
      ...
      const likes = values.Likes && values.Likes.length > 0 ? values.Likes.reduce((a, b) => a + b, 0)  : 0;
      const comments =values.Comments && values.Comments.length > 0 ? values.Comments.reduce((a, b) => a + b, 0)  : 0;
      const shares = values.Shares && values.Shares.length > 0 ? values.Shares.reduce((a, b) => a + b, 0)  : 0;
      ...
      if (selectedMetric.includes('Likes')) {
        result.Likes = likes;
      }
       if (selectedMetric.includes('Comments')) {
        result.Comments = comments;
      }
      if (selectedMetric.includes('Shares')) {
  ...
  const prepareTableData = (campaigns: any) => {
    ...
    const summarizedData = campaigns?.map((campaign: any) => {
      const metrics = campaign.creatives.reduce((totals: any, creative: any) => {
        if (creative.channel === "Youtube") {
          totals.Views += creative.Views || 0;
          totals.Clicks += creative.Clicks || 0;
          totals.Likes += creative.Like || 0;
          totals.Comments += creative.Comment || 0;
          totals.Shares += creative.Shares || 0;
  ...
  const columns = [
    ...
    columnHelper.accessor('Likes', { header: () => <div className='flex gap-10'><span>Likes</span></div> }),
    columnHelper.accessor('Comments', { header: () => <div className='flex gap-10'><span>Comments</span></div> }),
    columnHelper.accessor('Shares', { header: () => <div className='flex gap-10'><span>Shares</span></div> }),
  ...
  return (
    <Flex gap="4" wrap="wrap">
      <Flex gap="2" align="center" width="100%">
        ...
        <Heading as="h3" className='pagehead'>YouTube</Heading>
      </Flex>
                      ...
                      <Box className='boxy'>
                        <Flex direction="column" gap="2">
                          ...
                          <Flex direction="row" gap="3" align="center" >
                            <Box width={'50%'} style={{ textAlign: 'left' }}>
                              <Text>QoQ</Text><br></br>
                              <Text size='2' weight={'bold'}>+6%</Text>
                            </Box>
                            <Box width={'50%'} style={{ textAlign: 'right' }}>
                              <Text>YoY</Text><br></br>
                              <Text size='2' weight={'bold'}>+14%</Text>
                            </Box>
                          </Flex>
                        </Flex>
                      </Box>
                    ...
                    <Card variant="surface" className='card-content' mt='10px' >
                      ...
                      <Flex direction="row" gap={"3"} >
                        ...
                        <Box width='300px' mt='50px'>
                          ...
                          <Box mt='20px'>
                            <MetricFilters type={"Youtube"} campaignData={campaignData} updateData={selectedData} />
                          </Box>
                        </Box>
                        <Box style={{ width: '100%' }}>
                          <BarChartWithLegend
                  ...
                  <View UNSAFE_style={{marginTop:'30PX'}}>
                    ...
                    /> */}
                     {filteredData?.filter(campaign =>
                      ...
                      campaign.creatives.some(creative => creative.channel === "Youtube")
                    ).length === 0 && (
                        <Box mt='100px'>
                    ...
                    >
                     {filteredData?.filter(campaign => campaign.status === "Completed" && campaign.creatives.some(creative => creative.channel === "Youtube")).map((campaign,idx)=>{
                        const creativeGraphic = campaign.creatives.find(creative => creative.channel.includes("Youtube"))?.CreativeGraphic;
    ...
    selectedCampaign.creatives
        .filter(creative => creative.channel === "Youtube")
        .map((creative) => {
    ...
    <span>No data available</span>
        ...
        {selectedCampaign && (
          <img 
            src={selectedCampaign.creatives.find(creative => creative.channel === "Youtube")?.CreativeGraphic} 
