...
const Facebook: React.FC = () => {
  ...
  const calculateTotalReachForEmailChannel = (creativeData: CreativeCampaign[]): number => {
    return creativeData
      .filter(creativeData => creativeData.Channel === 'Facebook')
  ...
  const calculateEmailOR = (data: CreativeCampaign[]): number => {
    ...
    data.forEach(campaign => {
      campaign.Creatives.forEach(creative => {
        if (creative.OR !== null && creative.Channel == 'Facebook') {
          emailOR += creative.OR;
          count++
        }
      });
  ...
  const calculateEmailCTR = (data: CreativeCampaign[]): number => {
    ...
    data.forEach(campaign => {
      campaign.Creatives.forEach(creative => {
        if (creative.CTR !== null && creative.Channel == 'Facebook') {
  ...
  const calculateMarketSpend = (data: CreativeCampaign[]): number => {
    ...
    data.forEach(campaign => {
      campaign.Creatives.forEach(creative => {
        if (creative.MarketSpend !== null && creative.Channel == 'Facebook') {
  ...
  const prepareBarData = (campaignIdsToFilter: any[], selectedMetric: any) => {
    const filteredData = creativeData.filter(campaign => campaignIdsToFilter.includes(campaign.CampaignId) && campaign.Channel === "Facebook");
  ...
  return (
    <Flex gap="4" wrap="wrap">
      <Flex gap="2" align="center" width="100%">
        ...
        <Heading as="h3" className='pagehead'>Facebook</Heading>
      </Flex>
                    ...
                    <Card variant="surface" className='card-content' mt='10px' >
                      ...
                      <Flex direction="row" gap={"3"} >
                        ...
                        <Box width='300px' mt='50px'>
                          ...
                          <Box mt='20px'>
                            <MetricFilters type={"Facebook"} campaignData={campaignData} updateData={selectedData} />
                          </Box>
                        </Box>
                        <Box style={{ width: '100%' }}>
                          <BarChartWithLegend
              ...
              <Tabs.Content value="library">
                <Box width="100%" style={{ margin: '10px 0' , minHeight:'400px'}}>
                  ...
                  {filteredData?.filter(campaign =>
                      ...
                      campaign.creatives.some(creative => creative.channel === "Facebook")
                    ).length === 0 && (
                        <Box mt='100px'>
                    ...
                    >
                      {filteredData?.filter(campaign => campaign.status === "Completed" && campaign.creatives.some(creative => creative.channel === "Facebook")).map((campaign,idx)=>{
                        const creativeGraphic = campaign.creatives.find(creative => creative.channel.includes("Facebook"))?.CreativeGraphic;
    ...
    selectedCampaign.creatives
        .filter(creative => creative.channel === "Facebook")
        .map((creative) => {
    ...
    <span>No data available</span>
        ...
        {selectedCampaign && (
          <img 
            src={selectedCampaign.creatives.find(creative => creative.channel === "Facebook")?.CreativeGraphic} 
